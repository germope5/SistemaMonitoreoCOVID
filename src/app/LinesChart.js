...
import 'moment/locale/en-gb'; // Establece el idioma de moment a inglés británico
import '../public/App.css'; // Importa los estilos de la aplicación
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
  ...
  Legend,
  Filler,
} from 'chart.js'; // Importa varios elementos y escalas de Chart.js
ChartJS.register(
  CategoryScale,
  LinearScale,
  PointElement,
...
const LinesChart = () => {
  const [selectedCollection, setSelectedCollection] = useState('Mayo 2020'); // Estado para almacenar la colección seleccionada
  const [data, setData] = useState([]); // Estado para almacenar los datos de tweets
  const [chartData, setChartData] = useState(null); // Estado para almacenar los datos del gráfico

  moment.locale('en-gb'); // Establece el idioma de moment
  ...
  // Objeto para almacenar datos de tweets por día del mes
  // const dataByMonth = {
  //   'Marzo_2020': {
  ...
  // Este efecto se ejecuta cuando se cambia la colección seleccionada
  useEffect(() => {
    if (selectedCollection) {
      // Realiza una solicitud HTTP para obtener los datos de tweets de la colección seleccionada
      axios
        .get(`/miapi/collection/${selectedCollection}`)
        .then((response) => {
  ...
  // Este efecto se ejecuta cuando cambian los datos de tweets o la colección seleccionada
  useEffect(() => {
    if (data.length > 0) {
      // Objeto para contar la cantidad de tweets por día
      const dayCounts = {};

      // Itera sobre los datos de tweets y cuenta la cantidad de tweets por día
      data.forEach((item) => {
        try {
          const fechaMoment = moment(
            item.fecha,
            'ddd MMMM DD HH:mm:ss Z YYYY'
          );
          if (fechaMoment.isValid()) {
            const day = fechaMoment.format('DD');
            const month = fechaMoment.format('MMM');
            const year = fechaMoment.format('YYYY');
            const formattedDate = `${day} / ${month} / ${year}`;

            if (!dayCounts[formattedDate]) {
              dayCounts[formattedDate] = 0;
            }
            dayCounts[formattedDate]++;
          }
        } catch (error) {
  ...
  return (
    <div className="lines-chart-container">
      <div className="period-selector">
        ...
        <select onChange={(e) => setSelectedCollection(e.target.value)}>
          ...
          {colecciones.map((collection) => (
            // Genera opciones de selección para las colecciones disponibles
      ...
      <div className="chart-wrapper">
        {/* Renderiza el gráfico de líneas si hay datos disponibles */}
...
export default LinesChart; // Exporta el componente LinesChart